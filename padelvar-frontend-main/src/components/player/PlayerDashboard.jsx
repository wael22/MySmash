...
import TutorialModalTrigger from '../tutorial/TutorialModalTrigger';  // ✅ Auto-trigger modals
import { useTutorialDemo } from '../../hooks/useTutorialDemo';  // ✅ Tutorial demo data
...
const PlayerDashboard = () => {
  const { user } = useAuth();
  const { mergeData, isDemoMode, shouldShowDummy, dummyVideos } = useTutorialDemo();  // ✅ Tutorial demo
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="mt-6">
          ...
          {activeTab === 'videos' && (
            <div>
              ...
              ) : dashboardData.videos.length > 0 || shouldShowDummy(dashboardData.videos) ? (
                <div id="videos-section" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {mergeData(dashboardData.videos).map((video) => (
                    <VideoCardModern
                      key={video.id}
                      video={video}
      ...
      <TutorialModalTrigger
        onOpenRecording={() => setIsRecordingModalOpen(true)}
        onOpenProfile={() => {/* Profile modal non implémenté encore */ }}
        onOpenSupport={() => setActiveTab('support')}
        onOpenCredits={() => setIsBuyCreditsModalOpen(true)}
        onOpenClipEditor={() => {
          // Ouvrir l'éditeur avec la première vidéo démo si disponible
          if (mergeData(dashboardData.videos).length > 0) {
            const demoVideo = mergeData(dashboardData.videos)[0];
